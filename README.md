...
エクスポートは注文のあった日しか教えてくれないので、最終日の翻日以降が
「売上ゼロ」なのか「まだデータが無い」のか区別できない。`build_payload.py` の `ASOF` に
「この日まで確認済み」を入れると、その日までを対象期間にして日次グラフにゼロの印を出す。
データの最終日が `ASOF` より後ならデータ側を採るので、更新を忘れても期間が短くはならない。
環境変数 `ASOF=2026-09-24` でも指定できる。
...
元データの `payment_method` は表記ゆれを含めて 15 種類あるので、`build_payload.py` の `PAYGRP` で QR決済 / カード / 現金 / 銀行振込・ネットバンキング にまとめている。
画面では大分類を出し、行をクリックすると内訳が開く。未登録の決済方法は「その他・未記録」に
落ち、集計時に警告が出るので `PAYGRP` に足すこと。Apple Pay は店頭の非接触カード決済とみて
カードに入れている。
